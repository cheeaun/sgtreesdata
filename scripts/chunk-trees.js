...
const trees = {};
fs.readdir('data/grid', (e, files) => {
  const grids = files.filter((file) => /^grid\-/i.test(file));

  grids.forEach((grid) => {
    console.log(`Reading data/grid/${grid}`);
    const data = JSON.parse(fs.readFileSync(`data/grid/${grid}`));
  ...
  for (id in trees) {
    const { geometry, properties } = trees[id];
    ...
    const heritage =
      type === 'heritage' ||
      (ACTUAL_TREE_TYPE || '').toLowerCase() === 'heritage';
    const flowering =
      type === 'flowering' || (FLOWERING || '').toLowerCase() === 'yes';
    if (heritage) count.heritage++;
    if (flowering) count.flowering++;

    // if height is `null`, assume 1
    const height_est = parseInt(
      ((height || '0').match(/(\d+)[^\d]*$/) || [, 0])[1],
      10,
    );
    ...
    if (treeCoords[centerStr]) {
      console.warn(
        `📍  Tree ${tree_id} already exists with EXACT SAME location.`,
      );
